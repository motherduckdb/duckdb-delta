...
static void GetWriteDataFunction(DataChunk &input, ExpressionState &state, Vector &output) {
    ...
    BinaryExecutor::Execute<string_t, string_t, bool>(input.data[0], input.data[1], output, input.size(), [&](string_t path, string_t blob) {
        ...
        auto file = fs.OpenFile(path.GetString(), FileOpenFlags::FILE_FLAGS_FILE_CREATE | FileOpenFlags::FILE_FLAGS_WRITE);
...
ScalarFunctionSet DeltaFunctions::GetWriteFileFunction(ExtensionLoader &loader) {
	...
	ScalarFunction write_file({LogicalType::VARCHAR, LogicalType::BLOB}, {LogicalType::BOOLEAN}, GetWriteDataFunction, nullptr, nullptr);
