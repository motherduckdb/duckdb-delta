...
//
// TODO: (@benfleis) manual copy of recently added Path helper; after v1.5.0, delete this
// and ref DuckDB core's version
//
...
public:
	...
	// does this have `ancestor` in its parentage? If n == -1, any depth allowed. If n >= 0, it must match at exactly
	// that depth, e.g. n=1 Path("/a/b/c/d.txt").HasParentage("/a/b/c", n=1) -> true, but n!= -> false.
	bool HasParentage(const Path &ancestor, int n = -1);
	...
	// TODO: vet the APIs here before moving into DuckDB; quick adds for Delta

	Path ToLocal() const;     // file:/{1,3}... -> /..., throws if non-local
	Path ToFileLocal() const; // /... -> file:///..., throws if non-local
