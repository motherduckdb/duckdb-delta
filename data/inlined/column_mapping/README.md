...
Hand-written Delta logs, used by `test/sql/inlined/column_mapping/write_field_ids.test` and, for the
`invalid_*` ones, `invalid_annotations.test`. Each is a single `metaData` action whose `schemaString`
carries `delta.columnMapping.physicalName` and `delta.columnMapping.id` per field, plus a `protocol`
action enabling the `columnMapping` feature.
